...
public class Zarr3CachedFS3Test extends N5Zarr3Test {
  ...
  protected N5Zarr3Writer createN5Writer(final String location,
    ...
    return createN5Writer(location, new GsonBuilder(), chunkKeyEncoding, true);
  }
  ...
  protected N5Reader createN5Reader(final String location, final GsonBuilder gson)
    ...
    return new N5Zarr3Reader(location, gson, true);
  }
  ...
  protected N5Zarr3Writer createN5Writer(
    ...
    return new N5Zarr3Writer(location, gsonBuilder, chunkKeyEncoding,
  ...
  public void cachedRootDatasetTest() throws IOException {
    ...
    final String testDirPath = tmpPathName("zarr-cached-test-");
    try (Zarr3KeyValueWriter writer = (Zarr3KeyValueWriter) createN5Writer(testDirPath,
        new GsonBuilder())) {
      writer.createDataset("/", dimensions, blockSize, DataType.UINT8, getCompressions()[0]);
      assertTrue(writer.exists("/"));
    }

    try (Zarr3KeyValueReader reader = (Zarr3KeyValueReader) createN5Reader(testDirPath,
        new GsonBuilder())) {
  ...
  public void cacheTest() throws IOException {
    ...
    final String cachedGroup = "cachedGroup";
    try (Zarr3KeyValueWriter zarr = (Zarr3KeyValueWriter) createN5Writer()) {
      zarr.createGroup(cachedGroup);
      final String attributesPath = new File(zarr.getURI()).toPath()
          .resolve(cachedGroup)
          .resolve(Zarr3KeyValueReader.ZARR_JSON_FILE)
    ...
    try (final Zarr3KeyValueWriter zarr = createN5Writer()) {
      zarr.createGroup(cachedGroup);
  ...
  public void cacheBehaviorTest() throws IOException, URISyntaxException {
    ...
    try (final ZarrTrackingStorage n5 = new ZarrTrackingStorage(keyValueAccess, loc,
  ...
  public static class ZarrTrackingStorage extends Zarr3KeyValueWriter implements TrackingStorage {
    ...
    public ZarrTrackingStorage(final KeyValueAccess keyValueAccess, final String basePath,
      ...
      super(keyValueAccess, basePath, gsonBuilder, true, new DefaultChunkKeyEncoding('.'),
          cacheAttributes);
    }
    ...
    @Override
    public boolean isGroupFromAttributes(final String normalCacheKey,
    ...
    @Override
    public void writeAttributes(final String normalGroupPath, final JsonElement attributes)
        throws N5Exception {
      writeAttrCallCount++;
